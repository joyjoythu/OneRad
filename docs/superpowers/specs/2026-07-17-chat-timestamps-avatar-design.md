...
- **写回方式（实现中确定）**：统一走 `_aupdate_state_preserving_resume` 助手——优先裸 `aupdate_state`，让 langgraph 依据 checkpoint 的 `versions_seen` 推断写入归属；仅当裸更新抛 `InvalidUpdateError`（仅发生在未经任何节点执行的 input-only checkpoint，不可能有挂起中断）时回退 `as_node="__start__`（其 writer 为纯 ChannelWrite、无分支函数；`call_llm` 的 writer 含 `should_continue`，空消息历史上会 IndexError，不可用）。**禁止**无条件 `as_node`：它会把更新伪装成该节点的写入，使 interrupt 处挂起的任务被跳过，confirm/cancel 恢复静默失效、线程卡死在待确认状态（实现期间实际踩到并有回归测试锁定）。`update_plan` 端点同用此助手（修复全新会话 500）。
- **调用点**（保证各路径收敛）：
