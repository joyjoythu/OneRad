...
- **AI / 工具消息**：由图内部节点在运行期间产生，无法在创建点逐个打标。新增 `_ensure_message_timestamps(graph, config)`：
  ...
  3. 仅当有改动时 `aupdate_state` 写回 checkpoint。
- **调用点**（保证各路径收敛）：
  - `_stream_agent` 的 `finally` 里 best-effort 调用（try/except 吞掉异常）——覆盖正常完成、异常、中途取消的运行；
  - `/stop` 端点补打未应答 tool_call 的 ToolMessage 之后调用一次。
