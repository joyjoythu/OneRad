...
def test_run_analysis_empty_paths():
    logs, report = _run_analysis("", "./clinical.csv", "./out", "auto", "", "", "deepseek-chat", "https://api.deepseek.com/v1")
    assert "不能为空" in logs
...
def test_run_analysis_success():
    ...
    with patch("app.ui.Orchestrator", return_value=mock_orch), patch("app.ui.register_default_handlers") as mock_reg:
        logs, report = _run_analysis("./img", "./clinical.csv", "./out", "auto", "", "", "deepseek-chat", "https://api.deepseek.com/v1")
