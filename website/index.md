...
OneRad 是一个基于大语言模型（LLM）的端到端影像组学分析平台。它把影像组学研究中的多个环节——

**格式转换（DICOM → NIfTI，按需）→ 图像/Mask 配对 → 参数确认 → 特征提取 → 临床表解析 → 特征筛选 + 建模 → 统计报告生成**
...
| 全流程分析 | 「开始分析」 | 探索 →（按需格式转换）→ 配对 → 参数 → 提取 → 临床表 → 建模 → 统计报告 |
